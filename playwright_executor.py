...
if __name__ == "__main__":
    ...
    # 1. Open site
    {"action": "open", "target": "https://opensource-demo.orangehrmlive.com/",
     "assertions": ["expect(page).to_have_title(re.compile('OrangeHRM'))"]},

    # 2. Fill username
    {"action": "fill", "params": {"selector": "input[name='username']", "value": "Admin"},
     "assertions": []},

    # 3. Fill password
    {"action": "fill", "params": {"selector": "input[name='password']", "value": "admin123"},
     "assertions": []},
    ...
    # 5. Click PIM menu
    {"action": "click", "target": "a[href*='pim']",
     "assertions": [
         "expect(page).to_have_url(re.compile('pim'))"
     ]},

    # 6. Logout
    {"action": "click", "target": ".oxd-userdropdown-tab",
     "assertions": []},

    {"action": "click", "target": "a[href*='logout']",
     "assertions": [
    ...
    try:
        ...
        result = executor.execute_test(
            test_steps,
            test_name="OrangeHRM — HR Workflow Automation"
        )
    except Exception as e:
